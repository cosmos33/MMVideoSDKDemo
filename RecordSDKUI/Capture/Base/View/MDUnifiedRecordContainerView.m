...
- (MDRecordExposureAdjustSlider *)exposureSlider {
    if (!_exposureSlider) {
        _exposureSlider = [[MDRecordExposureAdjustSlider alloc] init];
...
- (void)slider:(MDRecordExposureAdjustSlider *)slider value:(CGFloat)value {
    [self.delegate exposureBias:value];

    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideSlider:) object:slider];
    [self performSelector:@selector(hideSlider:) withObject:slider afterDelay:1.5];
}
