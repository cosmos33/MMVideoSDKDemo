...
- (void)downloadItem:(MDFaceDecorationItem *)item withType:(MDFaceDecorationDownloadType)type
{
    ...
    if (![self isDownloadingItem:item urlStr:urlStr]) {
        ...
        [[MDRecordFaceDecorationItemDownloader shared] downloadItem:item completion:^(MDFaceDecorationItem * _Nullable item, NSError * _Nullable error) {
            if (item) {
...
//        [[MDRecordContext fileReceiverManager] cancelReceiver:zipPath];
        [[MDRecordFaceDecorationItemDownloader shared] cancelWithItem:item];
