...
static CGFloat const kMusicTrimmerViewHeight = 69;

@interface MDMusicEditPalletController ()
<MDBeautyMusicVolumeViewDelegate,
MDMusicEditPalletHandlerDelegate,
MDNewMusicDownloadManagerDelegate,
NewMusicTrimViewDelegate,
...
@property (nonatomic, strong) NewMusicTrimView *trimMusicView;
...
- (void)_handleSelectedMusicCollectionItem:(MDMusicCollectionItem *)item indexPath:(NSIndexPath *)indexPath {
    ...
    if ([item resourceExist]) {
        ...
        [[MDNewMusicDownloadManager shared] download:item.musicVo bind:self];
    }
}
...
//    _trimView = [[MDSMSelectIntervalProgressView alloc] initWithFrame:CGRectMake(7, 76, MDScreenWidth - 14, kMusicTrimmerViewHeight)];
...
//    _trimView.selectAreaBgColor = RGBACOLOR(255, 255, 255, 0.08);
//    [cutEditBgView addSubview:_trimView];
//
//    __weak __typeof(self) weakSelf = self;
...
//    }];
    ...
    _trimMusicView = [[NewMusicTrimView alloc] initWithFrame:CGRectMake(7, 76, MDScreenWidth - 14, 70)];
...
- (void)valueChangedWithView:(NewMusicTrimView *)view startPercent:(CGFloat)startPercent endPercent:(CGFloat)endPercent {
    NSTimeInterval duration = CMTimeGetSeconds(self.currentDuration);
    if (duration == 0) {
        return;
    }
