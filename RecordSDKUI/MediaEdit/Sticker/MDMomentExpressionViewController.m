...
- (void)requestMomentExpressions {
    if (self.isDynamicDecoratorMode) {
        ...
        [MDRecordExpressionLoader loadDynamicExpressionWithCompletion:^(NSString * _Nullable json, NSError * _Nullable error) {
            if (json && !error) {
        ...
        [MDRecordExpressionLoader loadStaticExpressionWithCompletion:^(NSString * _Nullable json, NSError * _Nullable error) {
            if (json && !error) {
                NSDictionary *dateDict = [NSJSONSerialization JSONObjectWithData:[json dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:nil];
                [self requestSuccess:dateDict];
            } else {
