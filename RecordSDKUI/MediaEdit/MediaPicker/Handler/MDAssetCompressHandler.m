...
- (AVMutableVideoComposition *)createVideoCompositionWithAsset:(AVAsset *)asset targetSize:(CGSize)targetSize {
    ...
    CGAffineTransform(^transformForVideoTrack)(AVAssetTrack *) = ^CGAffineTransform(AVAssetTrack *videoTrack) {
        CGAffineTransform transform = videoTrack.preferredTransform;
        
        // bugfix: https://github.com/rs/SDAVAssetExportSession/pull/70
        if (transform.tx == -560) {
            transform.tx = 0;
        }
        ...
        CGFloat videoAngleInRadian = atan2(transform.b, transform.a);
        CGSize transformedSize = CGSizeApplyAffineTransform(naturalSize, videoTrack.preferredTransform);
        transformedSize = CGSizeMake(ABS(transformedSize.width), ABS(transformedSize.height));
        
        float xRatio = targetSize.width / transformedSize.width;
        float yRatio = targetSize.height / transformedSize.height;
        float ratio = MAX(xRatio, yRatio);
        
        CGAffineTransform translateTransform = CGAffineTransformMakeTranslation(targetSize.width / 2, targetSize.height / 2);
        CGAffineTransform scaleTransform = CGAffineTransformScale(translateTransform, ratio, ratio);
        CGAffineTransform rotateTransform = CGAffineTransformRotate(scaleTransform, videoAngleInRadian);
        CGAffineTransform appliedTransform = CGAffineTransformTranslate(rotateTransform, -naturalSize.width / 2, -naturalSize.height / 2);
    ...
    CGAffineTransform transform = transformForVideoTrack(videoTrack);
